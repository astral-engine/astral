...
pub(super) type ResourceLoader<R, P> = Arc<dyn Fn(P) -> Result<R> + Send + Sync + 'static>;
pub(super) type AssetLoader<R, P> =
	Arc<dyn Fn(P, &mut (dyn Read)) -> Result<R> + Send + Sync + 'static>;

pub(super) enum Closures<R, P> {
...
impl<R, P> Closures<R, P> {
	pub(super) fn new_resource<F>(loader: F) -> Self
	where
	...
	pub(super) fn new_asset<F>(loader: F) -> Self
	where
