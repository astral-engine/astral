...
pub(super) struct StaticRefVector<'a, T> {
...
impl<'a, T> StaticRefVector<'a, T> {
	...
	pub(super) unsafe fn push(&self, value: &'a T) -> NonZeroU32 {
	...
	pub(super) unsafe fn get_unchecked(&self, index: NonZeroU32) -> &'a T {
	...
	pub(super) fn get(&self, index: NonZeroU32) -> Option<&'a T> {
...
impl<T> Drop for StaticRefVector<'_, T> {
	fn drop(&mut self) {
...
unsafe impl<T> Send for StaticRefVector<'_, T> {}
unsafe impl<T> Sync for StaticRefVector<'_, T> {}
