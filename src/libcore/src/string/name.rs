	...
	H: BuildHasher,
{
	...
	///
	/// [`from_utf8_unchecked`]: astral_core::string::Name::from_utf8_unchecked
	///
	/// # Errors
	///
	...
	/// [`from_utf8_unchecked`]: astral_core::string::Name::from_utf8_unchecked
	/// [`from_utf8`]: astral_core::string::Name::from_utf8
	///
	/// # Examples
	///
	/// Basic usage:
...
impl<'system, H> Name<'system, H> {
	...
	///
	/// use astral::core::string::{Name, StringId};
	///
...
mod test {
	...
	fn test_from_utf8() {
		...
		let system = System::new(&engine);
		let string_system = Subsystem::new(64, &system);
		let xs = b"hello";
		assert_eq!(
			Name::from_utf8(xs, &string_system).unwrap(),
			Name::new("hello", &string_system)
		);

		let xs = "ศไทย中华Việt Nam".as_bytes();
		assert_eq!(
			Name::from_utf8(xs, &string_system).unwrap(),
			Name::new("ศไทย中华Việt Nam", &string_system)
		);
	}
	...
	fn test_from_utf8_lossy() {
		...
		let string_system = Subsystem::new(64, &system);
		let xs = b"hello";
		assert_eq!(Name::from_utf8_lossy(xs, &string_system), "hello");

		let xs = "ศไทย中华Việt Nam".as_bytes();
		let ys = "ศไทย中华Việt Nam";
		assert_eq!(Name::from_utf8_lossy(xs, &string_system), ys);

		let xs = b"Hello\xC2 There\xFF Goodbye";
		assert_eq!(
			Name::from_utf8_lossy(xs, &string_system),
			Name::new("Hello\u{FFFD} There\u{FFFD} Goodbye", &string_system)
		);
		...
		assert_eq!(
			Name::from_utf8_lossy(xs, &string_system),
			Name::new("\u{FFFD}foo\u{FFFD}\u{FFFD}bar", &string_system)
		);

		let xs = b"\xF1foo\xF1\x80bar\xF1\x80\x80baz";
		assert_eq!(
			Name::from_utf8_lossy(xs, &string_system),
			Name::new("\u{FFFD}foo\u{FFFD}bar\u{FFFD}baz", &string_system)
		);

		let xs = b"\xF4foo\xF4\x80bar\xF4\xBFbaz";
		assert_eq!(
			Name::from_utf8_lossy(xs, &string_system),
			Name::new("\u{FFFD}foo\u{FFFD}bar\u{FFFD}\u{FFFD}baz", &string_system)
		);
	...
	fn test_from_utf16() {
		...
		let pairs: [(Name<'_>, Vec<u16>); 5] = [(Name::new("𐍅𐌿𐌻𐍆𐌹𐌻𐌰\n", &string_system),
                 ...
                 (Name::new("\u{20000}", &string_system), vec![0xD840, 0xDC00])];

		for p in &pairs {
			let (s, u) = (*p).clone();
			let s_str = s.as_str();
			let s_as_utf16 = s_str.encode_utf16().collect::<Vec<u16>>();
			let u_as_string = Name::from_utf16(&u, &string_system).unwrap().as_str();

			assert!(std::char::decode_utf16(u.iter().cloned()).all(|r| r.is_ok()));
			assert_eq!(s_as_utf16, u);

			assert_eq!(u_as_string, s);
			assert_eq!(Name::from_utf16_lossy(&u, &string_system), s);

			assert_eq!(Name::from_utf16(&s_as_utf16, &string_system).unwrap(), s);
	...
	fn test_utf16_invalid() {
		...
		// lead + eof
		assert!(Name::from_utf16(&[0xD800], &string_system).is_err());
		// lead + lead
		assert!(Name::from_utf16(&[0xD800, 0xD800], &string_system).is_err());

		// isolated trail
		assert!(Name::from_utf16(&[0x0061, 0xDC00], &string_system).is_err());

		// general
		assert!(Name::from_utf16(&[0xD800, 0xd801, 0xdc8b, 0xD800], &string_system).is_err());
	}
	...
	fn test_from_utf16_lossy() {
		...
		// lead + eof
		assert_eq!(
			Name::from_utf16_lossy(&[0xD800], &string_system),
			Name::new("\u{FFFD}", &string_system)
		);
		// lead + lead
		assert_eq!(
			Name::from_utf16_lossy(&[0xD800, 0xD800], &string_system),
			Name::new("\u{FFFD}\u{FFFD}", &string_system)
		);

		// isolated trail
		assert_eq!(
			Name::from_utf16_lossy(&[0x0061, 0xDC00], &string_system),
			Name::new("a\u{FFFD}", &string_system)
		);

		// general
		assert_eq!(
			Name::from_utf16_lossy(&[0xD800, 0xd801, 0xdc8b, 0xD800], &string_system),
			Name::new("\u{FFFD}𐒋\u{FFFD}", &string_system)
		);
	}
	...
	fn test_from_iterator() {
		...
		let s = Name::new("ศไทย中华Việt Nam", &string_system);
		let t = "ศไทย中华";
		let u = "Việt Nam";
