	...
	Idx: PrimUnsignedInt,
{
	...
	pub(super) fn index(&mut self) -> Idx {
		debug_assert!(self.free());
		unsafe { self.entry.index() }
	}

	pub(super) fn set_index(&mut self, index: Idx) -> Option<T> {
	...
	Idx: Debug + PrimUnsignedInt,
{
	fn fmt(&self, fmt: &mut Formatter<'_>) -> fmt::Result {
		...
		unsafe {
			if self.occupied() {
				let _ = debug.field("value", self.entry.value());
			} else if !self.reserved() {
				let _ = debug.field("next_free", &self.entry.index());
			}
		}
		let _ = debug.field("version", &self.version());
		debug.finish()
	}
}
